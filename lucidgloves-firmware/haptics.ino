...
void scaleLimits(int* hapticLimits, float* scaledLimits){
  for (int i = 0; i < 5; i++){
    scaledLimits[i] = 160.0f - hapticLimits[i] / 1000.0f * 160.0f;
  }
}
...
void writeServoHaptics(int* hapticLimits){
  ...
  if(scaledLimits[0] >= 0 && scaledLimits[0] <= 160) thumbServo.write(scaledLimits[0]);
  if(scaledLimits[1] >= 0 && scaledLimits[1] <= 160) indexServo.write(scaledLimits[1]);
  if(scaledLimits[2] >= 0 && scaledLimits[2] <= 160) middleServo.write(scaledLimits[2]);
  if(scaledLimits[3] >= 0 && scaledLimits[3] <= 160) ringServo.write(scaledLimits[3]);
  if(scaledLimits[4] >= 0 && scaledLimits[4] <= 160) pinkyServo.write(scaledLimits[4]);
}
